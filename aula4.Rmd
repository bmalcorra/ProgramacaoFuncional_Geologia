...
- pacote tidypaleo
- abstrações e funções otimizadas para dados estrasti
...
distance <- read.csv("santos_2009.csv") %>% 
  as_tibble()
```

- Pergunta 1: qual relação do V em função da distância ate o sill?
...
distance %>%
  ...
  labs(y = "Depth (m)",
       title = "Permian Paraná Basin",
       caption="V changes from distance to sill, Santos et al., 2009")+
  theme_paleo()
...
```{r}
dados %>% ggtern(aes(x=Ab,y=An,z=Or))+
  geo_point()
```
