...
- 1) Importar banco de dados "volcano"  
- 2) Inspecionar colunas  
- 3) Inspecionar tipos de variáveis  
...
```{r}
volcano %>% 
  dplyr::select(volcano_name, country, latitude:elevation) %>% 
  head()
```
...
```{r}
volcano %>% 
  dplyr::select(-tectonic_settings) %>% 
  head()
```
...
```{r}
volcano %>% 
  dplyr::select(volcano_name, country, last_eruption_year) %>% 
  filter(!country == "Antarctica") %>% 
...
```{r}
volcano %>% 
  dplyr::select(volcano_name, country, latitude:elevation, last_eruption_year) %>% 
...
```{r}
volcano %>% 
  dplyr::select(volcano_name, country) %>% 
...
- 4) Quais os países nos quais os vulcões apresentam maior risco para a população local (até 5 km)? Fltre países em que existam mais de 2 vulcões.
...
```{r}
volcano %>% 
  dplyr::select(region, volcano_name, last_eruption_year) %>% 
...
```{r}
volcano %>% 
  dplyr::select(country, population_within_5_km) %>% 
  group_by(country) %>% 
  summarise(n = n(),
...
volcano_long <- volcano %>% 
  dplyr::select(volcano_name, latitude, longitude) %>% 
...
```{r}
volcano %>% 
  dplyr::select(volcano_name, latitude, longitude, last_eruption_year) %>% 
...
```{r}
volcano %>% 
  dplyr::select(volcano_name, tectonic_settings) %>% 
...
```{r}
volcano %>% 
  dplyr::select(volcano_name, volcano_number) %>% 
...
- 1) Qual o número de tipos de vulcões por país? Dica: count() + pivot_wider()
...
```{r}
volcano %>% 
  dplyr::select(country, primary_volcano_type, major_rock_1) %>% 
